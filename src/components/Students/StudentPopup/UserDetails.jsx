  ...
  return (
    <div className="flex gap-5 justify-between mt-8 max-w-full text-base font-medium w-[383px] px-8">
      ...
      <div className="flex flex-col items-start text-black">
        <div className="self-stretch">{userInfo.email}</div>
        <div className="mt-2.5">{userInfo.phone}</div>
        <div className="mt-2.5">{userInfo.gender}</div>
        <div className="mt-2.5">{userInfo.from}</div>
        <div className="mt-2.5">{userInfo.birthday}</div>
      </div>
    </div>
  );
}
